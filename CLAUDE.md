...
- **Package Manager**: npm (used for all commands)
...
├── img/                      # Images, logos
└── fonts/                    # Inter font files for OG image generation
...
- Required frontmatter: `title`, `description`, `tags`, `keywords`
- Tags reference `blog/tags.yml` (format: `label`, `permalink`, `description`)
- Authors reference `blog/authors.yml`
- RSS/Atom feeds are auto-generated at `/rss.xml` and `/atom.xml`
...
### OG Image Generation
OG images are auto-generated at build time via `@acid-info/docusaurus-og` plugin (Satori-based).
- Renderer: `src/og-image-renderer.js` — dark theme card with title, description, and tags
- Fonts: `static/fonts/Inter-Regular.ttf` and `Inter-Bold.ttf` (supports Cyrillic)
- Output: `build/preview-images/` directory, 1200×630px PNG
- No need to specify `image` in frontmatter — plugin generates and injects `og:image` meta tags automatically

### Comments System
Comments are powered by Giscus and stored in GitHub Discussions:
- Category: "Комментарии" (Comments)
- Repository: https://github.com/BazZziliuS/docusaurus/discussions
- Users can comment on posts and docs via GitHub authentication
...
- Theme configuration (navbar, prism, custom CSS)
  - Prism syntax highlighting for: `python`, `bash`, `lua`, `nginx` (in addition to defaults)
- Plugin setup:
  - Yandex Metrica (counter ID: 101940986)
  - Google Analytics (GTM-MBTPCD7S)
  - Algolia Search (appId: P1AFP1WSPF, indexName: blog_cloudea_org_p1afp1wspf_pages)
  - Backlinks plugin (`@koroligor/docusaurus-plugin-backlinks`)
  - Ideal Image plugin (quality: 85, max: 2000px, min: 500px, steps: 4, enabled in dev)
  - Giscus (`@giscus/react`) for comments via GitHub Discussions
  - OG Image Generation (`@acid-info/docusaurus-og`) — auto-generates OG preview images for blog posts and docs
- i18n: default locale `ru`, with `en` support
- SEO: JSON-LD structured data for the site, preconnect hints, theme colors
- Sitemap: weekly updates, ignores `/tags/**` and `/page/**`
...
4. Ensure `tags` exist in `blog/tags.yml` with format: `label`, `permalink`, `description`
5. Verify all internal links and anchors are correct (build will fail otherwise)
6. Use `/img/...` for images in `static/img/`
7. For large images, use Ideal Image plugin imports for optimization
8. Add author reference if not using default `bazzzilius`
